...
export const runAnalyzeIssueCommentsCount = async () => {
    ...
    let commitsCounts = {}
    for (const pr of repository.issues) {
        if (compareWithAgeLimit(new Date(pr.createdAt))) {
            const totalCommentsAndReviews = pr.comments.totalCount
    ...
    console.log(chalk.green("count of comments: ") + chalk.red("count of such Issues"))
